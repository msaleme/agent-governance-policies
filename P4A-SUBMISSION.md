...
| Lib tests | 48 pass | 74 pass |
| Every prior-review ask (A-M) addressed | OK (issue #6 wired + tested) | OK (issue #6 wired + tested) |
...
- **Category / injection point / scope:** Security / inbound / `api,resource` → applicability **MCP + A2A + API/LLM**
- **Catalog copy:** use `definition/gcl.yaml` `metadata.labels.description` verbatim (publish-ready;
  P4A catalog copy is **frozen at submission time**, so get it right up front). One-line hook for the
  listing summary: *"Proves the executed MCP/A2A action is the one the accompanying approval actually
  approved — six ABV v0.1 predicates checked at the gateway."*
- **Config surface:** approvalSource(header|rpc-param) / approvalHeader / approvalRpcField /
  executorHeader / requiredPredicates(P1-P6) / attesterKeys / clockSkewSeconds / mode(block|monitor) /
...
- **Framework refs (supporting-measure, never certification):** NIST SP 800-53 Rev5 AC-3/AC-4/AU-10/AU-2;
  OWASP LLM06 Excessive Agency; MITRE ATLAS + Engage (design vocabulary); EU AI Act Art 14. Full text in README.
- **Honesty boundary to carry into the listing:** the corpus/policy tests whether the RECORD proves
  approved==executed; P5's separate attester is what keeps the check meaningful. No `.on_response` handler.
...
5. Submit; reviewer **Tommaso Bolis** runs the same review loop the three decoy policies went through.
   Because we proactively addressed that review's full ask corpus (A-M, incl. issue #6 policy-violation
   registration), expect fewer round-trips.
